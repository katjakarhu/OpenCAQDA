...
Maybe split into multiple services (FileService, CodeService, NoteService)
...
def add_file_content(file_path, new_file, session, user):
    f = open(file_path, 'rb')
...
def save_coded_text(coded_text):
    session = DatabaseConnectivity().create_new_db_session()
...
def get_coded_texts(data_file_id, name):
    session = DatabaseConnectivity().create_new_db_session()
...
def load_binary_file_content(datafile):
    session = DatabaseConnectivity().create_new_db_session()
    ...
    def save_files(self, file_list):
        ...
        for file_path in file_list:
            ...
            if file_path.exists():
                ...
                add_file_content(file_path, new_file, session, user)
